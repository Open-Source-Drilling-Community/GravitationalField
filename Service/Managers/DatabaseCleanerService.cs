...
namespace NORCE.Drilling.GravitationalField.Service.Managers
{
    public class DatabaseCleanerService : BackgroundService
    {
        ...
        private readonly string[] _dataTables = ["GravitationalFieldCalculationOrderTable"];
        ...
        /// </summary>
        /// <returns>true if older GravitationalField were successfully deleted</returns>
        private bool CleanDatabase(string dataTable)
        {
            ...
            if (connection != null)
            {
                ...
                {
                    _logger.LogError(ex, "Impossible to clean old GravitationalField from {table}", dataTable);
                }
            }
            else
            {
