...
namespace NORCE.Drilling.GravitationalField.Service.Managers
{
    ...
    public class SqlConnectionManager
    {
        ...
        public static readonly string DATABASE_FILENAME = "GravitationalField.db";
        ...
        // 1) to avoid loading the complete GravitationalFieldCalculationOrder (heavy weight data) each time we only need contextual info on the data (light weight data)
        // 2) to keep control of the logic of inserting and selecting a light data in the database
        //    localized at the controller/manager level (storing GravitationalFieldCalculationOrderLight as a whole could induce database corruption issues)
        // If the light weight data concept is not implemented, the same contextual info can be retrieved directly from the GravitationalFieldCalculationOrder
        private readonly static Dictionary<string, string[]> _tableStructureDict = new Dictionary<string, string[]>()
            {
                { "GravitationalFieldTable", new string[] {
                    "ID text primary key",
                    "MetaInfo text",
                    "Type text",
                    "GravitationalField text" }
                },
                { "GravitationalFieldCalculationOrderTable", new string[] {
                    "ID text primary key",                    
                    "MetaInfo text",                    
                    // beginning of list of fields used only when light weight concept is implemented
                    "GravitationalFieldCalculationOrderLight text",
                    // end of list of fields used only when light weight concept is implemented
                    "CreationDate text",
                    "LastModificationDate text",
                    "GravitationalFieldCalculationOrder text" }
                }
            };
