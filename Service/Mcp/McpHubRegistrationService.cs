...
{
    public static readonly Guid ServiceTypeId = Guid.Parse("d0cbe782-f4da-48ef-9235-e5e89a4aa4b5");

    private const string InstanceIdFileName = "gravitationalfield-mcp-hub-instance-id.txt";
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    _logger.LogInformation("Registered GravitationalField MCP endpoint on MCP hub at {HubUri}", collectionUri);
                }
                else
                {
    ...
    private static McpHubRegistration CreateRegistration(McpHubOptions options, Guid instanceId)
    {
        ...
        string mcpHttpUrl = $"{publicBaseUrl}/GravitationalField/api/mcp";
        string mcpWebSocketUrl = ToWebSocketUrl($"{publicBaseUrl}/GravitationalField/api/mcp/ws");

        return new McpHubRegistration(
            ServiceTypeId,
            instanceId,
            string.IsNullOrWhiteSpace(options.ServiceName) ? "GravitationalField" : options.ServiceName,
            mcpHttpUrl,
            mcpWebSocketUrl,
